...
/**
 * In dev the client is served by Vite on its own port and has to be told where
 * the server is. Built, it is served by that server, so its own origin is the
 * right answer. Set VITE_WS_URL only to point a build at some other host.
 */
const fallback = import.meta.env.DEV
  ? `${scheme}://${location.hostname}:8080`
  : `${scheme}://${location.host}`;

const serverUrl: string = import.meta.env.VITE_WS_URL ?? fallback;
