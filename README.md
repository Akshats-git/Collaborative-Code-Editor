...
| --- | --- | --- |
| Web app | Vercel | Static build, nothing to keep alive |
| WebSocket server | Render | Needs a process that holds sockets open |
| Postgres | Neon | Free tier, and it speaks plain Postgres |
| Redis | Upstash | Free tier, and it speaks plain Redis |

### Why the server is not on Vercel

Vercel Functions do support WebSockets, but a function has a timeout — 60 seconds
on Hobby — and the socket dies with it. A collaborative session is a connection
that stays open for hours. That is not a limit to work around; it is a signal
that the workload wants a long-lived process, which is what Render provides.

### Server → Render

[`render.yaml`](render.yaml) is a blueprint: point Render at the repo and it
builds [`apps/server/Dockerfile`](apps/server/Dockerfile) from the workspace root.
`AUTH_SECRET` is generated once and held by Render; the three URLs are pasted in.

| Variable | Value |
| --- | --- |
| `NODE_ENV` | `production` |
| `AUTH_SECRET` | generated by Render, identical on every instance |
| `DATABASE_URL` | the Neon URL, **with `?sslmode=require`** |
| `REDIS_URL` | the Upstash URL (`rediss://…`) |
| `CORS_ORIGIN` | the deployed frontend origin |

Then apply the schema once, from anywhere that can reach the database:

```bash
DATABASE_URL='postgres://…?sslmode=require' npm run db:setup -w @cce/server
```

`sslmode` has to be in the URL. `pg` reads it from there and will otherwise
connect without TLS, which Neon refuses.
...
[`vercel.json`](vercel.json) builds the shared protocol package and then the web
app from the repository root. One variable:

| Variable | Value |
| --- | --- |
| `VITE_WS_URL` | `wss://your-server.onrender.com` |

`wss://`, not `ws://`: the page is served over HTTPS and a browser will not open
an insecure socket from a secure page. `VITE_API_URL` is derived from it by
swapping the scheme, so it only needs setting if the HTTP API lives elsewhere.
